...
function mergeUserData(raw) {
  ...
  if (!merged.hiddenRecommendations || typeof merged.hiddenRecommendations !== "object") merged.hiddenRecommendations = {};
...
function recommendMainPool(enemy, userData) {
  ...
  return Array.from(pickSet)
    .filter((champ) => champ !== enemy)
    .filter((champ) => !(userData?.hiddenRecommendations?.[enemy] || []).includes(champ))
    .map((champ) => {
      const m = getMatchup(champ, enemy);
      const adj = clampScore(userData?.scoreAdjustments?.[matchupKey(champ, enemy)] || 0);
      ...
      return {
        ...
        isManual,
        isHidden: (userData?.hiddenRecommendations?.[enemy] || []).includes(champ)
      };
    })
    .sort((a, b) => b.score - a.score)
    .slice(0, 16);
...
function HiddenRecommendationEditor({ enemyChamp, userData, setUserData }) {
  if (!enemyChamp) return null;
  const hiddenList = userData?.hiddenRecommendations?.[enemyChamp] || [];

  const restore = (champ) => {
    setUserData((prev) => {
      const old = prev.hiddenRecommendations?.[enemyChamp] || [];
      const next = old.filter((x) => x !== champ);
      const all = { ...(prev.hiddenRecommendations || {}) };
      if (next.length === 0) delete all[enemyChamp];
      else all[enemyChamp] = next;
      return { ...prev, hiddenRecommendations: all };
    });
  };
  ...
  return (
    <div className="rounded-2xl border border-red-500/30 bg-red-950/10 p-3">
      <div className="mb-2 text-sm font-black text-red-100">숨긴 추천픽</div>
      <p className="mb-2 text-xs text-slate-400">현재 상대({enemyChamp}) 추천 리스트에서 숨긴 챔피언. 누르면 복구됨.</p>
      <div className="flex flex-wrap gap-1">
        {hiddenList.map((champ) => (
          <button key={champ} onClick={() => restore(champ)} className="rounded-full bg-red-500/20 px-2 py-1 text-[11px] font-bold text-red-100">
            {champ} 복구
          </button>
        ))}
      </div>
...
function RecCard({ item, onPick, selected, onAdjust, onHide }) {
  return (
    <div className={cx("w-full rounded-2xl border p-3 text-left", selected ? "border-indigo-300 bg-indigo-500/20" : "border-slate-800 bg-slate-900")}>
      ...
      <div className="mt-2 flex items-center gap-1.5 border-t border-slate-800 pt-2">
        ...
        <button onClick={() => onAdjust(item.champ, "reset")} className="rounded-lg bg-slate-800 px-2 py-1 text-[11px] font-bold text-slate-300">초기화</button>
        <button onClick={() => onHide(item.champ)} className="ml-auto rounded-lg bg-red-950 px-2 py-1 text-[11px] font-bold text-red-100">숨김</button>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const hideRecommendation = (champ) => {
    if (!enemyChamp) return;
    setUserData((prev) => {
      const old = prev.hiddenRecommendations?.[enemyChamp] || [];
      return {
      ...
      <div className="mx-auto max-w-5xl space-y-3">
        ...
        {enemyChamp && (
          <section className="space-y-3 rounded-2xl border border-slate-800 bg-slate-950 p-3">
            ...
            <HiddenRecommendationEditor enemyChamp={enemyChamp} userData={userData} setUserData={setUserData} />
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{mainRecs.map((r) => <RecCard key={r.champ} item={r} onPick={setMyChamp} selected={myChamp === r.champ} onAdjust={adjustScore} onHide={hideRecommendation} />)}</div>
          </section>
        )}
