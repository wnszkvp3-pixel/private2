...
function recommend({ally, enemy, bans, pool, champMap}){
  ...
  for(const c of candidateNames){
    ...
    for(const e of enemyTopCandidates){
      ...
      if(guide){
        const plus = Math.round(32 * e.weight);
        score += plus;
        reasons.push(`상대 탑 후보 ${e.name} 상대로 카운터(+${plus})`);
        guideBlocks.push({ enemy:e.name, guide });
      } else if(topMeta[e.name] && e.weight >= 0.7) {
        if((m.tags.includes("탱커처리") || m.type==="duelist") && topMeta[e.name].tank>=2){ score += 8; reasons.push(`${e.name} 같은 탱커/브루저 처리 가능`); }
        if(m.type==="ranged" && ["bruiser","tank"].includes(topMeta[e.name].type)){ score += 7; reasons.push(`${e.name} 상대로 원거리 주도권 가능`); }
      }
    }

    if(!allyA.hasTank && m.tank>=3){ score += 16; reasons.push("우리 팀 탱커 부족 보완"); }
    else if(allyA.tankCount===0 && m.tank>=2){ score += 9; reasons.push("앞라인 부족 보완"); }

    if(allyA.apCount===0 && (m.damage==="AP" || m.damage==="mixed")){ score += 14; reasons.push("우리 팀 AP 딜 부족 보완"); }
    if(allyA.adCount===0 && (m.damage==="AD" || m.damage==="mixed")){ score += 10; reasons.push("우리 팀 AD 딜 부족 보완"); }
    if(allyA.engageCount===0 && m.engage>=2){ score += 13; reasons.push("우리 팀 이니시 부족 보완"); }
    if(allyA.peelCount===0 && m.peel>=2){ score += 6; reasons.push("아군 보호/받아치기 보완"); }

    if(enemyA.adHeavy && (m.tank>=2 || c==="말파이트")){ score += 12; reasons.push("상대 AD 비중 높음"); }
    if(enemyA.tankyEnemies>=2 && (m.tags.includes("탱커처리") || ["피오라","그웬","베인","카밀"].includes(c))){ score += 13; reasons.push("상대 앞라인/탱커 처리 가능"); }
    if(enemyA.diveThreat && (m.peel>=2 || ["그라가스","뽀삐","케넨","말파이트"].includes(c))){ score += 9; reasons.push("상대 돌진 조합 대응"); }
    if(enemyA.rangedCount>=2 && (m.engage>=2 || ["말파이트","오른","케넨"].includes(c))){ score += 8; reasons.push("상대 원거리 조합에 이니시 가능"); }
...
function ChampButton({champ, selected, disabled, onClick}){
  ...
  return <button disabled={disabled} onClick={onClick}
    className={cx("rounded-2xl border p-2 text-sm transition active:scale-95 disabled:cursor-not-allowed",
    ...
    <div className="mx-auto mb-1 flex h-11 w-11 items-center justify-center overflow-hidden rounded-full bg-slate-800 font-black">
...
function SelectModal({title, champions, selected, blocked, onSelect, onClose, multi=false, currentPool=[]}){
  ...
  const list=champions.filter(c=>c.name.includes(q) || c.id.toLowerCase().includes(q.toLowerCase()));
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-2">
    <div className="flex h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-3xl border border-slate-700 bg-slate-950">
      <div className="border-b border-slate-800 bg-slate-950 p-4">
        ...
        <div className="flex items-center gap-2 rounded-2xl bg-slate-900 px-3 py-2"><Search className="h-4 w-4 text-slate-400"/><input value={q} onChange={e=>setQ(e.target.value)} placeholder="챔피언 검색" className="w-full bg-transparent text-white outline-none"/></div>
        <p className="mt-2 text-xs text-slate-400">{multi ? "내 챔피언 풀은 여러 개 선택 가능" : "빨간색은 밴/이미 선택되어 선택 불가"}</p>
      </div>
      <div className="grid flex-1 grid-cols-4 gap-2 overflow-y-scroll overscroll-contain p-4 sm:grid-cols-6 md:grid-cols-8">
...
function Slot({label,value,onClick,tone="blue"}){
  ...
  return <button onClick={onClick} className={cx("min-h-20 rounded-2xl border p-3 text-left",colors)}>
    <div className="text-xs text-slate-400">{label}</div><div className="mt-2 font-bold text-white">{value||"선택"}</div>
  </button>
}
...
function Draft({champions, champMap, onStartGame}){
  ...
  return <div className="space-y-5">
    <section className="rounded-3xl border border-slate-800 bg-slate-950 p-4">
      <div className="mb-3 flex items-center gap-2 text-white"><ShieldBan className="h-5 w-5 text-amber-300"/><h2 className="text-lg font-bold">밴 선택</h2></div>
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">{bans.map((v,i)=><Slot key={i} label={`밴 ${i+1}`} value={v} tone="ban" onClick={()=>open("ban",i)}/>)}</div>
    </section>

    <section className="grid gap-4 md:grid-cols-2">
      <div className="rounded-3xl border border-blue-900/60 bg-slate-950 p-4">
        <div className="mb-3 flex items-center gap-2 text-white"><Users className="h-5 w-5 text-blue-300"/><h2 className="text-lg font-bold">우리편 선택</h2></div>
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-5 md:grid-cols-1 lg:grid-cols-5">{ally.map((v,i)=><Slot key={i} label={roleLabels[i]} value={v} onClick={()=>open("ally",i)}/>)}</div>
      </div>
      <div className="rounded-3xl border border-red-900/60 bg-slate-950 p-4">
        <div className="mb-3 flex items-center gap-2 text-white"><Swords className="h-5 w-5 text-red-300"/><h2 className="text-lg font-bold">상대편 선택</h2></div>
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-5 md:grid-cols-1 lg:grid-cols-5">{enemy.map((v,i)=><Slot key={i} label={roleLabels[i]} value={v} tone="red" onClick={()=>open("enemy",i)}/>)}</div>
        <p className="mt-3 text-xs text-slate-400">상대가 탑인지 확실하지 않아도 전체 조합과 탑 가능성 가중치로 계산함.</p>
      </div>
    </section>

    <section className="rounded-3xl border border-emerald-900/60 bg-slate-950 p-4">
      ...
      <p className="mt-2 text-xs text-slate-400">추천 결과는 이 챔피언 풀 안에서 우선 계산함. 풀에 없는 챔피언은 추천하지 않음.</p>
    </section>

    <div className="flex flex-wrap gap-2">
      <button onClick={run} className="rounded-2xl bg-indigo-500 px-5 py-3 font-bold text-white">조합 보고 챔피언 추천</button>
      <button onClick={()=>onStartGame({ally,enemy})} className="rounded-2xl bg-emerald-500 px-5 py-3 font-bold text-white">인게임 화면으로</button>
    </div>

    {recs.length>0 && <section className="grid gap-4 lg:grid-cols-2">
      <div className="rounded-3xl border border-slate-800 bg-slate-950 p-4">
        <div className="mb-3 flex items-center gap-2 text-white"><Sparkles className="h-5 w-5 text-indigo-300"/><h2 className="text-lg font-bold">추천 결과</h2></div>
        <div className="space-y-2">{recs.map(r=><button key={r.pick} onClick={()=>setSel(r)} className={cx("w-full rounded-2xl border p-4 text-left",sel?.pick===r.pick?"border-indigo-400 bg-indigo-500/15":"border-slate-800 bg-slate-900")}>
          <div className="flex items-center justify-between"><div className="text-xl font-black text-white">{r.pick}</div><div className="rounded-full bg-slate-800 px-3 py-1 text-sm font-bold text-indigo-200">{r.score}점</div></div>
          ...
          <ul className="mt-2 space-y-1 text-sm text-slate-300">{r.reasons.slice(0,3).map((x,i)=><li key={i}>- {x}</li>)}</ul>
        </button>)}</div>
      </div>

      <div className="rounded-3xl border border-slate-800 bg-slate-950 p-4">
        <div className="mb-3 flex items-center gap-2 text-white"><Info className="h-5 w-5 text-slate-300"/><h2 className="text-lg font-bold">추천 이유 / 상대법</h2></div>
        {sel ? <div className="space-y-3">
          <div className="rounded-2xl bg-slate-900 p-3"><div className="font-bold text-white">{sel.pick}</div><ul className="mt-2 space-y-1 text-sm text-slate-300">{sel.reasons.map((x,i)=><li key={i}>- {x}</li>)}</ul></div>
          {sel.guideBlocks.length ? sel.guideBlocks.map(g=><div key={g.enemy} className="rounded-2xl bg-slate-900 p-3"><div className="font-bold text-red-200">{g.enemy} 상대법</div><ol className="mt-2 space-y-1 text-sm text-slate-200">{g.guide.map((x,i)=><li key={i}>{i+1}. {x}</li>)}</ol></div>) : <p className="rounded-2xl bg-slate-900 p-3 text-sm text-slate-400">직접 카운터 상대법 데이터는 아직 없지만, 조합 점수 기준으로 추천된 픽임.</p>}
...
function InGame({draft,onBack}){
  ...
  return <div className="space-y-4"><div className="flex items-center justify-between"><h2 className="text-xl font-bold text-white">인게임 점멸 타이머</h2><button onClick={onBack} className="rounded-2xl bg-slate-800 px-4 py-2 text-slate-200">픽창으로</button></div>
    <div className="grid gap-3 md:grid-cols-5">{enemies.map((n,i)=>{const left=timers[i]||0;return <div key={i} className="rounded-3xl border border-slate-800 bg-slate-950 p-4"><div className="text-xs text-slate-400">상대 {roleLabels[i]}</div><div className="mt-1 text-xl font-bold text-white">{n}</div><div className={cx("mt-4 rounded-2xl p-4 text-center text-3xl font-black",left?"bg-red-500/20 text-red-200":"bg-emerald-500/15 text-emerald-200")}>{left?formatTime(left):"READY"}</div><button onClick={()=>start(i,300)} className="mt-3 w-full rounded-2xl bg-red-500 px-3 py-3 font-bold text-white">점멸 사용</button><div className="mt-2 grid grid-cols-2 gap-2"><button onClick={()=>start(i,255)} className="rounded-xl bg-slate-800 px-2 py-2 text-xs text-slate-200">우통 4:15</button><button onClick={()=>reset(i)} className="rounded-xl bg-slate-800 px-2 py-2 text-xs text-slate-200"><RotateCcw className="mx-auto h-4 w-4"/></button></div></div>})}</div>
  </div>
}
...
export default function App(){
  ...
  return <main className="min-h-screen bg-slate-950 p-3 text-slate-100 sm:p-6"><div className="mx-auto max-w-6xl">
    <header className="mb-5 rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-900 to-slate-950 p-5"><div className="flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-indigo-500 text-white"><TimerReset/></div><div><h1 className="text-2xl font-black text-white">탑 밴픽 조합 추천 & 점멸 타이머</h1><p className="text-sm text-slate-400">전체 챔피언 밴픽 입력 → 조합/카운터/챔피언 풀 기준 탑 추천</p></div></div></header>
