...
import {
  ...
  ExternalLink, Save, Plus, Trash2, Edit3, CheckSquare, Settings
} from "lucide-react";
...
function MainPoolSettings({ userData, setUserData }) {
  const [query, setQuery] = useState("");
  const mainPool = userData.settings?.mainPool || DEFAULT_MAIN_POOL;
  const filtered = allChampions.filter((c) => matches(c, query));
  ...
  const reset = () => setUserData((prev) => ({ ...prev, settings: { ...(prev.settings || {}), mainPool: DEFAULT_MAIN_POOL } }));

  return (
    <section className="rounded-2xl border border-indigo-500/30 bg-indigo-950/20 p-3">
      <div className="mb-2 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <Settings className="h-4 w-4 text-indigo-200" />
          <h2 className="text-sm font-black text-white">주요픽 설정</h2>
        </div>
        <button onClick={reset} className="rounded-lg bg-slate-800 px-2 py-1 text-[11px] font-bold text-slate-200">기본값</button>
      </div>
