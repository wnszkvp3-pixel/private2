...
function recommendAllCounters(enemy) {
  ...
  return allChampions
    ...
    .filter((x) => x.data && !x.matchup.includes("당함") && (x.matchup.includes("카운터") || x.matchup.includes("유리") || x.score >= 70))
    .sort((a, b) => b.score - a.score)
    .slice(0, 10);
}
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 flex flex-wrap gap-1">
        {(item.data?.recommendTags || []).map((t) => (
          <span key={t} className="rounded-full bg-slate-800 px-2 py-0.5 text-[10px] font-bold text-slate-300">{t}</span>
        ))}
      </div>
