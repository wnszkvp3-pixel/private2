...
import {
  Search, RotateCcw, Sword, Star, ChevronDown, ChevronUp, BookOpen,
  ExternalLink, Save, Plus, Trash2, Edit3, CheckSquare, Settings, Download, Upload, Copy
} from "lucide-react";
import db from "../data/matchups.json";

const DEFAULT_MAIN_POOL = ["오른", "요릭", "말파이트", "문도 박사", "피오라", "잭스", "요네"];
const STORAGE_KEY = "lol_top_matchup_user_data_v2";
const OLD_STORAGE_KEY = "lol_top_matchup_user_data_v1";
...
function useLocalUserData() {
  ...
  React.useEffect(() => {
    ...
    const v1 = window.localStorage.getItem(OLD_STORAGE_KEY);
    setData(mergeUserData(safeParse(v2 || v1, defaultUserData())));
  }, []);
...
async function copyUserData(userData) {
  ...
  if (navigator.clipboard) {
    ...
    alert("내 데이터가 클립보드에 복사됐어. 다른 기기에서 가져오기에 붙여넣으면 됨.");
  } else alert("클립보드 복사가 제한돼. 파일 내보내기를 사용해줘.");
}
function importUserDataText(text, setUserData) {
  try {
    const parsed = JSON.parse(text);
    setUserData((prev) => mergeUserData({ ...prev, ...parsed, settings: { ...(prev.settings || {}), ...(parsed.settings || {}) } }));
    alert("가져오기 완료");
  } catch { alert("가져오기 실패: JSON 형식이 아님"); }
}
...
function MainPoolSettings({ userData, setUserData }) {
  ...
  const addCustomChampion = () => {
    ...
    setUserData((prev) => {
      ...
      return { ...prev, settings: { ...(prev.settings || {}), customChampions: Array.from(new Set([...oldCustom, champ])), mainPool: Array.from(new Set([...oldPool, champ])) } };
    });
    setNewChamp("");
  };
  const removeCustomChampion = (champ) => {
    setUserData((prev) => {
      const oldCustom = prev.settings?.customChampions || [];
      const oldPool = prev.settings?.mainPool || DEFAULT_MAIN_POOL;
      return { ...prev, settings: { ...(prev.settings || {}), customChampions: oldCustom.filter((x) => x !== champ), mainPool: oldPool.filter((x) => x !== champ) } };
    });
  };
  ...
  return (
    <section className="space-y-3 rounded-2xl border border-indigo-500/30 bg-indigo-950/20 p-3">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2"><Settings className="h-4 w-4 text-indigo-200" /><h2 className="text-sm font-black text-white">주요픽 설정 / 데이터 공유</h2></div>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-950 p-3">
        ...
        <p className="mb-2 text-xs text-slate-400">신규 챔프가 생기면 이름을 추가할 수 있음. 기본 매치업 데이터는 없지만 주요픽/추천 리스트/검색에 표시되고 개인 노트로 관리 가능.</p>
        <div className="flex gap-2"><input value={newChamp} onChange={(e)=>setNewChamp(e.target.value)} onKeyDown={(e)=>{ if(e.key==='Enter') addCustomChampion(); }} placeholder="예: 신규챔프명" className="min-w-0 flex-1 rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-white outline-none placeholder:text-slate-500" /><button onClick={addCustomChampion} className="rounded-xl bg-emerald-600 px-3 py-2 text-xs font-bold text-white">추가</button></div>
        {customChampions.length > 0 && <div className="mt-2 flex flex-wrap gap-1">{customChampions.map((c)=><button key={c} onClick={()=>removeCustomChampion(c)} className="rounded-full bg-yellow-500/20 px-2 py-1 text-[11px] font-bold text-yellow-100">{c} 삭제</button>)}</div>}
      </div>
      <div className="rounded-2xl border border-slate-800 bg-slate-950 p-3">
        <div className="mb-2 text-sm font-black text-white">내 데이터 공유/백업</div>
        <p className="mb-2 text-xs text-slate-400">서버 동기화는 아니지만 파일/복사로 폰·컴퓨터 간 옮길 수 있음.</p>
        <div className="flex flex-wrap gap-2">
          ...
          <label className="inline-flex cursor-pointer items-center gap-1 rounded-xl bg-slate-800 px-3 py-2 text-xs font-bold text-slate-100"><Upload className="h-4 w-4" /> 파일 가져오기<input type="file" accept="application/json,.json" className="hidden" onChange={(e)=>{ const file=e.target.files?.[0]; if(!file) return; const reader=new FileReader(); reader.onload=()=>importUserDataText(String(reader.result||''), setUserData); reader.readAsText(file); e.target.value=''; }} /></label>
        </div>
        <textarea value={importText} onChange={(e)=>setImportText(e.target.value)} placeholder="다른 기기에서 복사한 데이터를 붙여넣기" className="mt-2 min-h-20 w-full rounded-xl border border-slate-700 bg-slate-900 p-2 text-xs text-white outline-none placeholder:text-slate-500" />
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-950 p-3">
        ...
        <div className="mb-2 rounded-xl border border-slate-700 bg-slate-950 px-3 py-2"><div className="flex items-center gap-2"><Search className="h-4 w-4 text-slate-400" /><input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="주요픽 추가/삭제 검색" className="w-full bg-transparent text-sm text-white outline-none placeholder:text-slate-500" /></div></div>
        <div className="mb-2 flex flex-wrap gap-1">{mainPool.map((c)=><button key={c} onClick={()=>toggle(c)} className="rounded-full bg-indigo-500/20 px-2 py-1 text-[11px] font-bold text-indigo-100">{c} ×</button>)}</div>
        <div className="grid max-h-44 grid-cols-4 gap-1.5 overflow-y-auto pr-1 sm:grid-cols-6">{filtered.map((champ)=>{ const checked=mainPool.includes(champ); return <button key={champ} onClick={()=>toggle(champ)} className={cx("rounded-xl border px-1.5 py-2 text-[11px] font-bold leading-tight", checked ? "border-indigo-300 bg-indigo-600 text-white" : "border-slate-700 bg-slate-900 text-slate-200")}><div className="truncate">{checked ? "✓ " : ""}{champ}</div></button>; })}</div>
      </div>
    </section>
  );
}
...
export default function App() {
  ...
  const detail = useMemo(() => {
    ...
    return {
      ...
      matchup: "개인 데이터",
      matchupTier: "개인 데이터",
      coreSummary: "신규 추가 챔피언이 포함된 매치업이라 기본 데이터가 없음. 수정 영역에 직접 상대법을 작성해서 개인 데이터로 관리 가능.",
      myStrongTiming: "",
      enemyStrongTiming: "",
      myChampionCore: "",
      enemyCore: "",
  ...
  return (
    <main className="min-h-screen bg-slate-950 p-2 text-slate-100 sm:p-4">
      <div className="mx-auto max-w-5xl space-y-3">
        <header className="rounded-2xl border border-slate-800 bg-gradient-to-br from-slate-900 to-slate-950 p-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 sm:text-sm">주요픽 설정 · 원본 설명 수정 · 개인 노트 · 체크리스트 저장 가능</p>
            </div>
          </div>
        </header>
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={() => setSettingsOpen(!settingsOpen)} className="inline-flex items-center gap-2 rounded-xl bg-indigo-700 px-3 py-2 text-xs font-bold text-white"><Settings className="h-4 w-4" /> 주요픽 설정</button>
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          <ChampionPicker title="상대 탑 선택" value={enemyChamp} onChange={setEnemyChamp} accent="red" champions={getAllChampions(userData)} />
          <ChampionPicker title="내 챔피언 선택" value={myChamp} onChange={setMyChamp} accent="indigo" champions={getAllChampions(userData)} />
        </div>
        ...
        {enemyChamp && (
          <section className="space-y-3 rounded-2xl border border-slate-800 bg-slate-950 p-3">
            ...
            <p className="text-xs text-slate-400">내 주요픽 + 명확한 카운터 후보 + 내가 점수 올린 챔프가 표시됨. 카드를 누르면 내 챔피언으로 선택됨.</p>
